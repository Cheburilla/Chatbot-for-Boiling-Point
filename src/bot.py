...
dp.register_message_handler(handlers.process_hall_invalid, lambda message: message.text not in ["Малый зал", "Большой зал"], state=Form.hall)
dp.register_message_handler(callback=handlers.process_hall, state=Form.hall)
dp.register_message_handler(handlers.process_date_invalid, lambda message: not handlers.is_date(message.text) or not handlers.is_date_now(message.text), state=Form.date)
dp.register_message_handler(callback=handlers.process_date, state=Form.date)
dp.register_message_handler(handlers.process_beg_t_invalid, lambda message: not handlers.is_time(message.text), state=Form.begin_time)
dp.register_message_handler(callback=handlers.process_beg_t, state=Form.begin_time)
dp.register_message_handler(handlers.process_end_t_invalid, lambda message: not handlers.is_time(message.text), state=Form.end_time)
dp.register_message_handler(callback=handlers.process_end_t, state=Form.end_time)
dp.register_message_handler(handlers.process_phone_invalid, lambda message: not is_phone_number(message.text), state=Form.phone)
